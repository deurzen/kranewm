...
ipc_t::handle_ipc()
{
    ...
    char cli_msg[BUFSIZ];
    ssize_t n = recv(cli_fd, cli_msg, sizeof(cli_msg) - 1, 0);
    ...
    if (cli_fd > 0 && n > 0) {
        ...
        if (cli_stream)
            process_message(msg_tokens, cli_stream);
        else {
...
ipc_t::resolve_command(::std::vector<::std::string>& tokens)
{
    if (tokens.size() && m_ipcbinds.count(tokens.front())) {
        ...
        if (bind.is_bound()) {
            if (tokens.size() > 1)
                return fail_command("this command takes no arguments");

            return bind.get_bind();
        }

        if (tokens.size() > 2)
            return fail_command("too many arguments provided");
    ...
    return fail_command("command not found");
}
